...
users can being lifting their UTXO to be able to patriotic pate in Shadowchain
blocks and operation. The process of entering the shadowchain is referred to as
...
proposed said block to the set of live Lifted UTXOs before we can move onto the
...
\texttt{ProposeBlock} attempts to proposed the given Shadowchain block to the
set of live Lifted UTXOs. The algorithm returns $b=1$ is all of the
...
proposed Shadowchain block, it can move unt to executing the block as well as
committing it in the base Bitcoin blockchain: \\
...
necessayr witneses, and brodcast the Bitcoin transaction which commits the
...
itself, it's possilbe for a Shadowchain operator to compres several disticnt
Shadowchain blocks into a single instance that is equivalent to the succesive
state transtion funciton application on each distinct shadowchain block: \\

\texttt{CoalesceBlocks($\{B_{S_0}, \cdots, B_{S_N}\}$)} $\rightarrow$
$B_{S}^\prime$. Given a series of \emph{consecutive} Shadowchain blocks, the
\texttt{CoalesceBlocks} algorithms compresses each consqeutive block into a
single block $B_{S_N}$ which has an equivalent output state to the serial
appliation of the state transition functino on each induvidual Shadowchain
block. \\
...
The process of upgrading the chain in term sof the \emph{types} of
application-level transactions offered, or the set of valid state transition
functinos can be done entirely off-chin in a de-syhcnronied manner. In order to
update the environment, state transition functions, or the apicatin-level
...
$\rightarrow$ $\bot$. The \texttt{UpgradeChain} is a algorithm that's executed
entirely in an off-chain manner, than lets the Operator of the shadowchain
upgrade some or all of: the applciation transaciton data, the execution
environment, and the set of state transition functions. Due to the nature of
the shadowchain, in order to uitlize thsi new functinlaity each user must
update thieir local state. However, note that the Lifted UTXO remains the same,
as the Operator's long-term public key hasn't changed.

\subsubsection{Shadowchain Operation}

In this section, we'll put together the above algorithms to out-line the main
...
to the set of live UTXOs, filter our any participants that rejected the block,
...
new block proposal by the Shadowchain operator.
...
The above script can either be spent via the timee out clause using the
...
Now that we have our concrete account structure, we'll move onto presenting a
...
A lesser less trust-minimized versing of channel lease duration enforcement is
...
based enfrocement, the only direct option the \texttt{Liquidity Maker} has is
...
Given the existence of a new modified chanenl $C_{L}$ that supports the ch
annel lease protocol, we require a new way to fund the channels as a channel
...
\emph{expect} certain parameeters of the channel itself. Given the
...
P_{a}$, outPoint)} instructs a Lightning Node to use a \emph{prearragned} public key as
specified within channel lease $\Gamma$ for its portion of the multi-sig output
used wtihin the ultimate channel.

The existnce of the channel shim API allows a node to proceed in the set up of
...
commitment transaction in a manneer that doesn't incur any risk as due to the
account structure, they'll need to sign of on the batch funding trsnaction
itrself before it can br broadcast.

\subsubsection{Order Structure}

With our concrete account structure and channel lease semsntics in place, we'll
...
digest of the order.  This is done by concatenating each items of the order
into a single byte stream, with the set of $\Delta_{aux}$ attributes specifying
a custom serialization mechanism. 

For our tag generation, we opt to utilize an \seufcma signature scheme. As we
...
also require that all the backing Lightning nodes of the also include as
signature as well.  As the number of backing nodes for a given account may be
numerous, rather than accepting individual signatures for each node, we isntead
...
Given the \texttt{MuSig} multi-signature scheme, we define thee following
...
\begin{itemize}
    \item Let \texttt{Sign($\{P_{0}, \dots, P_{i}\}, M$)} $\rightarrow$
        ...
        multi-signature signed by the s et of public keys on message $M$.
...
\paragraph{Node Rating Agencies}

% rating agency, purpose and impls
% rating as aux variable in bos score

\paragraph{Batch Fee Rate Preference}
...
lift shadow chain stuff from blog post
