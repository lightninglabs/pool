...
# All keys that can sign pool releases. The key must be added as a file to the
...
TEMP_DIR=$(mktemp -d /tmp/pool-sig-verification-XXXXXX)
...
function import_keys() {
  ...
  # sure one user cannot just upload multiple signatures to reach the 2/4
  # required sigs.
  for key in "${KEYS[@]}"; do
...
# By default we're picking up poold and pool from the system $PATH.
POOLD_BIN=$(which poold)
POOL_BIN=$(which pool)

if [[ $# -eq 0 ]]; then
  echo "ERROR: missing expected version!"
  echo "Usage: verify-install.sh expected-version [path-to-poold-binary-or-download-archive [path-to-pool-binary]]"
  exit 1
fi
...
# If exactly two parameters are specified, we expect the first one to be poold 
# and the second one to be pool. One parameter is either just a single binary or
# a packaged release archive. No parameters means picking up poold and pool from
# the system path.
if [[ $# -eq 2 ]]; then
  POOL_BIN=$(realpath $1)
  POOLD_BIN=$(realpath $2)

  # Make sure both files actually exist.
  if [[ ! -f $POOLD_BIN ]]; then
    echo "ERROR: $POOLD_BIN not found!"
    exit 1
  fi
  if [[ ! -f $POOL_BIN ]]; then
    echo "ERROR: $POOL_BIN not found!"
    exit 1
  fi

  # Make sure both binaries can be found and are executable.
  check_command "$POOLD_BIN"
  check_command "$POOL_BIN"
  ...
  echo "Usage: verify-install.sh [poold-binary pool-binary]"
  exit 1
fi
...
if [[ "$PACKAGE_BIN" != "" ]]; then
  ...
  check_hash "$POOLD_BIN" "poold"
  check_hash "$POOL_BIN" "pool"

  echo ""
  echo "SUCCESS! Verified poold and pool against $MANIFEST signed by $NUM_CHECKS developers."
fi
