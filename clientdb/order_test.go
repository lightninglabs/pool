...
func TestUpdateOrders(t *testing.T) {
	...
	err = store.UpdateOrder(
		o1.Nonce(), order.StateModifier(order.StatePartialFill),
	)
	if err != nil {
	...
	if storedOrder.Details().State != order.StatePartialFill {
		t.Fatalf("unexpected order state. got %d expected %d",
			storedOrder.Details().State, order.StatePartialFill)
	}
